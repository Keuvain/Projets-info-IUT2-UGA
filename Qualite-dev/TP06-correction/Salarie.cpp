...
const std::string & Salarie::getNom() const {
    ...
    nomEnMajuscule = "";
    // Erreur à trouver : remplacer i=1 par i=0
...
void Salarie::setNumeroSS(const std::string & numeroSS) {
    ...
    // Erreur à trouver : remplacer numeroSS[0] >= '2' par numeroSS[0] > '2'
...
void Salarie::setSalaireMensuel(float salaireMensuel) {
    ...
    try {
        ...
        // Erreur à trouver : remplacer NumeroIncorrectException() par SalaireIncorrectException()
...
float Salarie::getImpot() const {
    ...
    for (auto it = TRANCHES_IMPOT.begin(); 
        ...
        // Erreur à trouver : remplacer revenuImposable <= it->first par revenuImposable < it->first
