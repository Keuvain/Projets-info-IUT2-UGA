...
// {n >= 0} => {résultat = x élevé à la puissance n}
    ...
    * À COMPLETER
    */
    if (n == 0) {           // base premier cas
        return 1;
    } else if (n == 1) {    // base deuxième cas
        return x;
    } else {                // application directe de la définition récursive
...
double puissanceNbMult(float x, int n, int &nbmult) {
    ...
    if (n == 0) {           // base premier cas
        ...
        nbmult = nbmult + 1;    // on fait une multiplication
...
double puissanceDrNbMult(float x, int n, int &nbmult) {
    ...
    if (n == 0) {               // base premier cas
        ...
        return x;
    } else if ((n % 2) == 0) {  // n est pair, application de la définition
        nbmult = nbmult + 1;
        // la manière astucieuse consiste à n'appeler qu'une seule fois puissanceDivReg
        // si on l'appelle deux fois, tout le bénéfice est perdu
        double temp = puissanceDrNbMult(x, int(n / 2), nbmult);
        return temp * temp;
    } else {                    // n est impair, application de la définition
        nbmult = nbmult + 2;
        // la manière astucieuse consiste à n'appeler qu'une seule fois puissanceDivReg
        // si on l'appelle deux fois, tout le bénéfice est perdu
        double temp = puissanceDrNbMult(x, int(n / 2), nbmult);
