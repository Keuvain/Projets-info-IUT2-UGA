...
private:
    ...
    bool estPliableRecWorker(const NoeudBinaire<TypeInfo>* ptrGauche, const NoeudBinaire<TypeInfo>* ptrDroit) const;
...
int ArbreNoeudBinaireRecherche<TypeInfo>::getHauteurRecWorker(NoeudBinaire<TypeInfo>* ptrRac) const {
    ...
    // NOTE (max() méthode fournie http://www.cplusplus.com/reference/algorithm/max/)
    // > ptrRac == null -> return 0 ; l'arbre vide est de hauteur 0
    // > ptrRac != null -> return 1 + max (getHauteurWorker(sous-arbre gauche), getHauteurRecWorker(sous-arbre droit));
    if (ptrRac == nullptr) {
        return 0;
    } else {
...
void ArbreNoeudBinaireRecherche<TypeInfo>::insertRecWorker(NoeudBinaire<TypeInfo>*& ptrRac, const TypeInfo& nouvelleInfo) {
    // appliquer la définiton récrusive de l'insertion
    // > ptrRac == null -> ptrRac  = nouveau noeud; *
    // > ptrRac != null ->
    //      >> nouvelleInfo <= ptrRac->getInfo()  -> insertion à gauche de ptrRac;
    //      >> nouvelleInfo > ptrRac->getInfo() -> insertion à droite de ptrRac;
    if (ptrRac == nullptr) {
...
TypeInfo ArbreNoeudBinaireRecherche<TypeInfo>::getAncetreCommunLePlusBasRecWorker(const NoeudBinaire<TypeInfo>* ptrRac,
    ...
    // si val1 et val2 sont toutes les deux plus petites que l'information portée par la racine
    // alors le plus bas ancêtre commu est à gauche
    if (val1 < ptrRac->getInfo() && val2 < ptrRac->getInfo()) {
        ...
        // si val1 et val2 sont toutes les deux plus grandes que l'information portée par la racine
        // alors le plus bas ancêtre commu est à droite
        return getAncetreCommunLePlusBasRecWorker(ptrRac->getPtrFilsDroit(), val1, val2);
    } else {
        // si val1 est inférieure ou égale à information portée par la racine
        //    et val2 est supérieure ou égale à l'information portée par la racine
        // alors le plus bas ancêtre commun est la racine
...
bool ArbreNoeudBinaireRecherche<TypeInfo>::aMemeGeometrieQueRecWorker(const NoeudBinaire<TypeInfo>* monPtrRac, const NoeudBinaire<TypeInfo>* sonPtrRac) const {
    ...
    // > monPtrRac == nullptr && sonPtrRac == nullptr => return true; *
    // {l'un au plus des deux arbres est vide}
    // > monPtrRac == nullptr || sonPtrRac == nullptr => return false; *
    // {aucun de deux arbres est vide}
    // > monPtrRac != nullptr || sonPtrRac != nullptr =>
    ...
    if (monPtrRac == nullptr && sonPtrRac == nullptr) {
        ...
        // les deux arbres ne sont pas vides
        return aMemeGeometrieQueRecWorker(monPtrRac->getPtrFilsGauche(), sonPtrRac->getPtrFilsGauche())
               && aMemeGeometrieQueRecWorker(monPtrRac->getPtrFilsDroit(), sonPtrRac->getPtrFilsDroit());
    }
}
...
TypeInfo ArbreNoeudBinaireRecherche<TypeInfo>::getMaxRecWorker(const NoeudBinaire<TypeInfo>* ptrRac) const {
    // {je n'ai pas de fils droit, je suis le plus grand}
    // > ptrRac->getPtrFilsDroit() == nullptr => return ptrRac->getInfo(); *
    // {sinon aller chercher le plus grand dans mon fls gauche}
    // > ptrRac->getPtrFilsDroit() != nullptr => return getMaxRecWorker(ptrRac->getPtrFilsDroit());
...
bool ArbreNoeudBinaireRecherche<TypeInfo>::aPourSousArbreRecWorker(const NoeudBinaire<TypeInfo>* ptrRacCetArbre, const NoeudBinaire<TypeInfo>* ptrRacUnArbre) const {
    if (ptrRacUnArbre==nullptr) {
        // l'autre arbre est vide
        return true;
    } else if (ptrRacCetArbre== nullptr) {
        // cet arbre est vide, l'autre non, pas de sous-arbre
        return false;
    } else if (estEgalARecWorker(ptrRacCetArbre, ptrRacUnArbre)) {
        // cet arbre et l'autre sont égaux
        return true;
    } else {
        // aller à droite ou à gauche selon la valeur de la racine de l'autre arbre par rapport à celle de cet arbre
        if (ptrRacUnArbre->getInfo() < ptrRacCetArbre->getInfo()) {
...
template<class TypeInfo>
bool ArbreNoeudBinaireRecherche<TypeInfo>::estPliableRecWorker(const NoeudBinaire<TypeInfo>* ptrGauche, const NoeudBinaire<TypeInfo>* ptrDroit) const {
    // si ptrGauche et ptrDroit == nullptr, c'est une feuille donc vrai
    if (ptrGauche == nullptr && ptrDroit == nullptr) {
        return true;

        // si l'un des deux est nullptr, retour faux
    } else if (ptrGauche == nullptr || ptrDroit == nullptr) {
        return false;

        // sinon vérifier que les sous-arbres droits et gauches sont miroirs de leur contre parties
    } else {
        return estPliableRecWorker(ptrGauche->getPtrFilsGauche(), ptrDroit->getPtrFilsDroit()) &&
               estPliableRecWorker(ptrGauche->getPtrFilsDroit(), ptrDroit->getPtrFilsGauche());
    }
}
