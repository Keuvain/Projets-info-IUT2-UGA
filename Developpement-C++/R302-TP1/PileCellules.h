...
PileCellules<TypeInfo>::PileCellules(const PileCellules<TypeInfo>& unePile) {
    ...
    if (ptrPileOriginale == nullptr) {
        ptrSommet = nullptr; // cette pile (la nouvelle) est videRec
    } else {
        // Copie du sommet
...
void PileCellules<TypeInfo>::depile() throw (PrecondVioleeExcep) {
    if (!estVide()) {
        ...
        Cellule<TypeInfo>* ptrSommetASupprimer = ptrSommet;
        ...
        ptrSommetASupprimer->setSuivante(nullptr);
        delete ptrSommetASupprimer;
