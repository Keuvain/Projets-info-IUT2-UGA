...
bool ListeChainee<TypeInfo>::insereAtPositIter(const int nouvellePosition, const TypeInfo &nouvelleInfo) {
    // ICI on vérifie que l'insertion est possible avant de la réaliser
    // Et on utilise la méthode qui retourne un pointeur sur la cellule qui précède la cellule
    // après laquelle insérer
    // L'insertion est-elle possible ?
    ...
    if (insertionPossible) {
        ...
        // insertion de la nouvelle cellule à la nouvellePosition
        if (nouvellePosition == 1) {
            // insertion de la nouvelle cellule en tête
            ptrNouvelleCellule->setSuivante(ptrTete);
            ptrTete = ptrNouvelleCellule;
        } else {
            // recherche de la cellule qui précéde la nouvelleCellule
            // Initialisation
            Cellule<TypeInfo>* ptrCellulePrecedente = ptrTete; // pointeur pour parcourir la liste
            int positionCourante = 1; //on veut s'arrêter sur la cellule au rang position

            while (positionCourante < nouvellePosition-1) {
                positionCourante++;
                ptrCellulePrecedente = ptrCellulePrecedente->getSuivante();
            }
            // positionCourante == nouvellePosition-1
            // insertion de la nouvelle cellule après la cellule qui précède
            // -> la cellule suivante de la nouvelle cellule est la cellule qui suit la cellule précédente
            ptrNouvelleCellule->setSuivante(ptrCellulePrecedente->getSuivante());
            // -> la cellule qui suit mainenant la cellule précédente est la nouvelle cellule
...
bool ListeChainee<TypeInfo>::supprimeAtPositIter(int position) {
    // Hypothèse : positionCourante <= position - 1
    // > ptrCellulePrecedente->getSuivante() == nullptr => return false; *
    // > ptrCellulePrecedente->getSuivante() != nullptr =>
    //          >> positionCourante == position - 1 => supprimer la suivante ; return true; *
    //          >> positionCourante < position - 1 => positionCourante++; ptrCellulePrecedente = ptrCellulePrecedente->getSuivante(); => H
    // Itération tq (ptrCellulePrecedente->getSuivante() != nullptr) && (positionCourante < position - 1) faire
    // Initialisation
    //          > ptrTete == nullptr (la liste est videRec) => retrun(false); *
    //          > ptrTete != nullptr =>
    //                  >> position == 1 => suprimeTête(ptrTete); retrun true; *
    //                  >> position != 1 => positionCourante = 1; ptrCellulePrecedante = ptrTete; => H
    if (ptrTete == nullptr) { // la liste est videRec
        return false;
    } else if (position == 1) { // suppression en tête
        // suppression en tête
        supprimeTeteWorker(ptrTete);
        return true;
    } else { // suppression plus loin
        // Initialisation
        Cellule<TypeInfo> *ptrCellulePrecedente = ptrTete; // pointeur pour parcourir la liste
        int positionCourante = 1; //on veut s'arrêter sur la cellule précédente

        while ((ptrCellulePrecedente->getSuivante() != nullptr) && (positionCourante < position - 1)) {
            positionCourante++;
            ptrCellulePrecedente = ptrCellulePrecedente->getSuivante();
        }
        // Tableau de sortie
        // ptrCellulePrecedente->getSuivante() == nullptr | positionCourante == position - 1 | Réponse
        //              vrai                              |     non examiné                  | return false
        //              faux                              |     vrai                         | supprimer la suivante et return true
        //              faux                              |     faux                         | impossible (tq))
        // si on est arrivé sur la cellule precédente et qu'elle a un successeur
        if ((ptrCellulePrecedente->getSuivante() != nullptr) && (positionCourante == position - 1)) {
            // pointeur sur la cellule à supprimer
            Cellule<TypeInfo> *ptrCelluleASupprimer = ptrCellulePrecedente->getSuivante();
            // la suivante de la précédente est la suivante de la suivante
            ptrCellulePrecedente->setSuivante(ptrCellulePrecedente->getSuivante()->getSuivante());
            // la cellule à supprimer n'a plus de successeur
...
bool ListeChainee<TypeInfo>::estInfoPresentRecWorker(Cellule<TypeInfo> *ptrCetteListe, TypeInfo &infoCible) const {
    //si la liste est videRec l'information n'est pas présente
    if (ptrCetteListe == nullptr) {
        return false;
    } else if (ptrCetteListe->getInfo() == infoCible) {
        //si l'information portée par la première Cellule est infoCible on a trouvé
        return true;
    } else {
        //sinon continuer
...
void ListeChainee<TypeInfo>::supprimeAtPositRecWorker(Cellule<TypeInfo>*& ptrCetteListe, int position) {
    // > position = 1  ->
    //          supprimeTête(ptrCetteListe);
    // > position != 1 ->
    //          supprimeAtPositRecFuncWorker(ptrCetteListe->getRefSuivante(), position - 1);
...
void ListeChainee<TypeInfo>::afficheDGIter() const {
    ...
    // on peut au choix empiler les informations ou les pointeurs sur les Cellules
    PileCellules<TypeInfo> maPile;

    // aller au bout de la liste en empilant
    Cellule<TypeInfo> *ptrCelluleCourante = ptrTete;
    while (ptrCelluleCourante) {
        // empiler l’information
        maPile.empile(ptrCelluleCourante->getInfo());
        ptrCelluleCourante = ptrCelluleCourante->getSuivante();
    }
    // aller au bout de la pile en affichant et dépilant
    while (!maPile.estVide()) {
        cout << maPile.consulteSommet() << " ";
        maPile.depile();
    }
    cout << endl;
}
